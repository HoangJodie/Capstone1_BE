...
export class AuthController {
  ...
  async login(@Req() req: Request, @Res() res: Response) {
    ...
    const { accessToken, refreshToken } = this.authService.generateTokens(user); // Tạo cả access và refresh token

    // Lưu refresh token vào cookies
    res.cookie('refreshToken', refreshToken, {
      httpOnly: true,  // Cookie chỉ có thể truy cập bởi server, không thể truy cập qua JS phía client
      secure: true,    // Cookie chỉ gửi qua HTTPS, bạn có thể tạm thời để false khi phát triển cục bộ
      sameSite: 'strict',  // Tùy chọn SameSite để tránh tấn công CSRF
      maxAge: 7 * 24 * 60 * 60 * 1000,  // Thời gian tồn tại của cookie (7 ngày)
    });

    return res.json({ accessToken }); // Trả về access token cho client
  }
